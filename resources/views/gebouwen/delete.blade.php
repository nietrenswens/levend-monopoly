        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('dashboard.gebouwen.destroy') }}" method="POST">
                        @csrf
                        <label for="gebouw_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Kies een team</label>
                        ...
                        @foreach ($gebouwen as $gebouw)
                            <option value="{{ $gebouw->id }}">{{ $gebouw->naam }}</option>
                        @endforeach
                        </select>
